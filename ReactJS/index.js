...
io.on('connection', function(socket) {
	...
	// check whether or not the room that this socket.id was in is empty. If empty, remove from game initiation
	...
	socket.on('taitiSubmitClient', function(data) {
		...
		if(gameInstanceIndex !== -1) {
			...
			if(data.passVote) {
				...
				//generate message
				msg = "Player " + (data.whoseTurn + 1) + " played a " + data.selected_type + '!';
